...
class MainContainer extends React.Component {
    ...
    deleateProduct = (id) => {
        axios.delete(`http://192.168.7.11:3001/products/${id}`)
            .then((response) => {
    ...
    updateProduct = (id, product) => {
        axios.patch(`http://192.168.7.11:3001/products/${id}`,{product: product})
            .then((response) => {
                const productIndex = this.state.products.findIndex(x => x.id === id)
    ...
    createProduct = (product) => {
        axios.post('http://192.168.7.11:3001/products/',{product: product} )
            .then((response) => {
